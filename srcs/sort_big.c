...
void	distance_in_stack_a(t_stack *stack)
{
	...
	while (i <= stack->b_size)
	{
		...
		temp->distance_in_a = find_best_spot(temp->data, stack->a,
								stack->a_size, ASCENDING);
		num = temp->distance_in_a > (stack->a_size / 2) ?
					stack->a_size - temp->distance_in_a : temp->distance_in_a;
...
void	sort_big(t_stack *stack)
{
	...
	while (stack->b_size)
	{
		...
		rotate_best_to_top(&stack->b, cheapest_node(stack->b),
							stack->b_size, 'b');
		rotate_best_to_top(&stack->a, stack->b->distance_in_a,
							stack->a_size, 'a');
