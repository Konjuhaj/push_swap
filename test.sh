...
VAR=$($push_swap $ARG_ZERO | $checker $ARG_ZERO | grep OK)
echo "Short test	-> 6 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_ZERO | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_ZERO > 0SHORT_TEST.txt

echo "----------------------------------------------------"

ARG_ONE="1 2 4 5 6 7 8 3 9  12 "
VAR=$($push_swap $ARG_ONE | $checker $ARG_ONE | grep OK)
echo "Short test	-> 10 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_ONE | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_ONE > 1SHORT_TEST.txt

echo "----------------------------------------------------"

ARG_SMALL="34 84 65 109 11 7 18 55 72 44 27 58 62 46 23 76 54 106 104 41"
VAR=$($push_swap $ARG_SMALL | $checker $ARG_SMALL | grep OK)
echo "Short2 test	-> 20 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_SMALL | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_SMALL > 2SHORT_TEST.txt

echo "----------------------------------------------------"

ARG_SMALL2="56 36 17 54 32 63 20 107 55 64 5 52 57 75 23 108 59 31 58 89"
VAR=$($push_swap $ARG_SMALL2 | $checker $ARG_SMALL2 | grep OK)
echo "Short3 test	-> 20 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_SMALL2 | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_SMALL > 3SHORT_TEST.txt

echo "----------------------------------------------------"

ARG_THREE="53 47 28 109 59 108 33 38 67 95 66 93"
VAR=$($push_swap $ARG_THREE | $checker $ARG_THREE | grep OK)
echo "Mid test	-> 12 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_THREE | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_THREE > 4SHORT_TEST.txt

echo "\n\n--------------------LONG TESTs-----------------------"

ARG_BIG2="56 36 17 54 32 63 20 107 55 64 5 52 57 75 23 108 59 31 58 89 1"
VAR=$($push_swap $ARG_BIG2 | $checker $ARG_BIG2 | grep OK)
echo "BIG1 test	-> 21 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_BIG2 | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_BIG2 > BIG1.txt
...
VAR=$($push_swap $ARG_MID | $checker $ARG_MID | grep OK)
echo "Mid-long test	-> 41 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_MID | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_MID > BIG2.txt
...
VAR=$($push_swap $ARG_TWO | $checker $ARG_TWO | grep 'OK')
echo "LONG test	-> 100 numbers"
if [ $VAR ]; then
	LINES=$($push_swap $ARG_TWO | wc -l)
	echo "		${GREEN}OK${NC}	linecount = $LINES"
else
		echo "${RED}FAIL${NC}"
fi
$push_swap $ARG_TWO > BIG3.txt
